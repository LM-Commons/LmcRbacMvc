...
is known to the ServiceManager as `'LmcRbac\Service\AuthorizationService'`. Here is a classic example for injecting
        ...
        return [
            'factories' => [
                 'MyService' => function($sm) {
                     $authService = $sm->get('LmcRbac\Service\AuthorizationService');
