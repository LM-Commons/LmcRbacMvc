...
class Module
{
    ...
    public function getServiceConfig()
    {
        return [
            'initializers' => [
                'LaminasRbac\Initializer\AuthorizationServiceInitializer'
            ]
        ];
    }
}
...
LaminasRbac is shipped with a `LaminasRbac\Factory\AuthorizationServiceDelegatorFactory` [delegator factory]
...
is known to the ServiceManager as `'LaminasRbac\Service\AuthorizationService'`. Here is a classic example for injecting
        ...
        return [
            'factories' => [
                 'MyService' => function($sm) {
                     $authService = $sm->get('LaminasRbac\Service\AuthorizationService');
...
LaminasRbac comes with both a controller plugin and a view helper to check permissions.
