...
LmcRbac is flexible enough to allow you to create custom guards. Let's say we want to create a guard that will
...
Now we just need to add the guard to the `guards` option, so that LmcRbac can execute the logic behind this guard. In
