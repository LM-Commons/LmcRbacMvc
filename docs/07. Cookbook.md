...
This section will help you further understand how LaminasRbac works by providing more concrete examples. If you have
...
- [Using LaminasRbac with Doctrine ORM](07.%20Cookbook.md#using-zfcrbac-with-doctrine-orm)
    - [How to deal with roles with lot of permissions?](07.%20Cookbook.md#how-to-deal-with-roles-with-lot-of-permissions)
- [Using LaminasRbac and ZF2 Assetic](07.%20Cookbook.md#using-zfcrbac-and-zf2-assetic)
- [Using LaminasRbac and ZfcUser](07.%20Cookbook.md#using-zfcrbac-and-zfcuser)
...
Assuming the application example above we can easily use LaminasRbac to protect our route using the following guard:
        ...
        return [
            'factories' => [
                 'PostService' => function($sm) {
                     return new PostService(
                         $sm->get('doctrine.entitymanager.orm_default'),
                         $sm->get('LaminasRbac\Service\AuthorizationService') // This is new!
                     );
                 }
            ]
        ];
...
is not the task of such service. The Permission-System should handle this. And we can, for this we have the
 [`AssertionPluginManager`](/LaminasRbac/Assertion/AssertionPluginManager.php) and here is how to do it:
...
## Using LaminasRbac with Doctrine ORM

First your User entity class must implement `LaminasRbac\Identity\IdentityInterface` :
...
You need one more configuration step. Indeed, how can the RoleProvider retrieve your role and permissions? For this you need to configure `LaminasRbac\Role\ObjectRepositoryRoleProvider` in your `zfc_rbac.global.php` file :
```php
        /**
...
Using DoctrineORM with LaminasRbac is very simple. You need to be aware of performance where there is a lot of permissions for roles.
...
## Using LaminasRbac and ZF2 Assetic

To use [Assetic](https://github.com/widmogrod/zf2-assetic-module) with LaminasRbac guards, you should modify your
...
return [
    'assetic_configuration' => [
        'acceptableErrors' => [
            \LaminasRbac\Guard\GuardInterface::GUARD_UNAUTHORIZED
        ]
    ]
];
```

## Using LaminasRbac and ZfcUser
