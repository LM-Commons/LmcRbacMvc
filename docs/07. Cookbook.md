...
This section will help you further understand how LmcRbac works by providing more concrete examples. If you have
...
- [Using LmcRbac with Doctrine ORM](07.%20Cookbook.md#using-lmcrbac-with-doctrine-orm)
    - [How to deal with roles with lot of permissions?](07.%20Cookbook.md#how-to-deal-with-roles-with-lot-of-permissions)
- [Using LmcRbac and ZF2 Assetic](07.%20Cookbook.md#using-lmcrbac-and-zf2-assetic)
- [Using LmcRbac and LmcUser](07.%20Cookbook.md#using-lmcrbac-and-lmcuser)
        ...
        return [
            'factories' => [
                 'PostService' => function($sm) {
                     return new PostService(
                         $sm->get('doctrine.entitymanager.orm_default'),
                         $sm->get('LmcRbac\Service\AuthorizationService') // This is new!
                     );
                 }
            ]
        ];
...
is not the task of such service. The Permission-System should handle this. And we can, for this we have the
 [`AssertionPluginManager`](/LmcRbac/Assertion/AssertionPluginManager.php) and here is how to do it:
...
You need one more configuration step. Indeed, how can the RoleProvider retrieve your role and permissions? For this you need to configure `LmcRbac\Role\ObjectRepositoryRoleProvider` in your `zfc_rbac.global.php` file :
```php
        /**
...
Using DoctrineORM with LmcRbac is very simple. You need to be aware of performance where there is a lot of permissions for roles.
